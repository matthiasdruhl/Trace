...
Indexed links for the core crates (**Lance**, **DuckDB**, **AWS SDK for S3**) used by the search engine are in [`docs/reference/RUST_CRATE_DOCS.md`](../reference/RUST_CRATE_DOCS.md).
...
Use `--force` when overwriting an existing table in the same directory. Upload to S3 is off by default (`--skip-upload`); see `python scripts/seed.py --help` and `docs/reference/DATA_SPEC.md` for column layout and optional staging or promotion flags.
